...
import type { CDRecord } from '@/fomu/record'
...
export function getSiblings<A extends BasicElement>(elem: A): A[] {
  if (!elem) return []
...
/** return node paths */
export function getPaths<A extends BasicElement>(elem: A): A[] {
  if (!elem) return []
...
export function createCopyName(set: Set<string>, name: string): string {
  if (!name) return ''
...
export function createElementRecursively<A extends BasicElement>(
...
export function extractPureNodeRecursively(
  a: CDRecord,
  option?: {
...
function isCoincide(a: Rectangle, b: Rectangle): boolean {
  return !(
    a.x + a.width < b.x ||
    a.x > b.x + b.width ||
    a.y > b.y + b.height ||
